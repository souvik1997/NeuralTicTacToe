...
describe 'Genome', ->
  ...
  it 'can find the number of differences
  between itself and other genomes', () ->
    network = new NeuralNetwork()
    sense_1 = new SensoryNeuron(id: 1)
    sense_2 = new SensoryNeuron(id: 2)
    hidden_3 = new Neuron(id: 3)
    ...
    genome.deconstruct(network)
    cloned_genome = genome.clone()
    cloned_genome.genes.push({empty: true})
    expect(cloned_genome.differences(genome)).toEqual(1)
